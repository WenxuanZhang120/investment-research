...
`financial_collection_plan.json` 固定完成高级财务指标所需的五个问句及报告期。问句文本、请求版本和预期范围是原始数据身份的一部分，续采时不得改写；沪深主板任务的最低覆盖数必须与 `investment_universe.json` 一致。计划运行器会从已有不可变快照推导下一页，并隔离同版本的坏响应，而不是人工记忆进度或每天从第一页重试。仅当精确问句声明“按股票代码升序排列”时，导入器才启用原始页内和跨页严格升序门；现有不含该标记的 legacy 任务不会被追溯性改变。新增或修改分页问句应加入该排序标记，同时提升 `request_version`并把上一版精确问句移入 `historical_queries`。

ETF 日采问句使用已由 Raw 响应验证能正确识别为“纳斯达克100ETF或标普500ETF”的短条件，避免把交易所文字误解析为持仓概念；查询仍必须保留完整字段清单、ETF 代码升序和完整分页门。
