...
import { getPortfolioSnapshot, getResearchSnapshot, refreshPortfolioQuotes } from "@/lib/github-research-source";
import type { ResearchSnapshot } from "@/lib/research-types";

export const dynamic = "force-dynamic";

const fallbackResearch = fallbackResearchJson as ResearchSnapshot;
...
function LoginScreen() {
  return <main className="login-shell light-login">
    <section className="login-intro">
      ...
      <div className="login-capabilities"><span><b>{fallbackResearch.coverage.screeningTotal.toLocaleString("zh-CN")}</b> 条筛选结果</span><span><b>{fallbackResearch.coverage.financialSecurityCount.toLocaleString("zh-CN")}</b> 只证券财务</span><span><b>{fallbackResearch.content.length}</b> 条研究内容</span></div>
    </section>
    <section className="login-panel">
      <div className="login-panel-inner"><span className="login-lock">PRIVATE ACCESS</span><h2>进入综合投资研究看板</h2><p>该站点保存个人持仓与决策日志，只允许指定 GitHub 账户访问。</p><form action={loginWithGitHub}><button className="github-login-button" type="submit"><span>GH</span>使用 GitHub 登录 <b>→</b></button></form><small>仅允许 <strong>WenxuanZhang120</strong></small><div className="login-data-note"><i /><span>持仓与研究数据直接读取 GitHub <strong>main</strong></span></div></div>
    </section>
  </main>;
}
...
export default async function Home() {
  ...
  const [logs, research, portfolio] = await Promise.all([getDecisionLogs(), getResearchSnapshot(), getPortfolioSnapshot()]);
  const refreshedPortfolio = refreshPortfolioQuotes(portfolio, research);
