    ...
    <section className="workspace-main">
      ...
      <div className="view-container">
        ...
        {view === "screening" && <section className="dashboard-view">
          ...
          <div className="surface-card screener-card"><div className="table-toolbar"><div><strong>{filteredSecurities.length.toLocaleString("zh-CN")}</strong><span> 条符合条件</span></div><small>点击任意证券查看行情、财务和评分</small></div><div className="data-table-wrap"><table className="research-table"><thead><tr><th>排名</th><th>证券</th><th>优先级</th><th>评分</th><th>PE TTM</th><th>总市值</th><th>净利率</th><th>现金流利润率</th><th>状态</th></tr></thead><tbody>{visibleSecurities.map((item) => <tr key={item.securityCode} onClick={() => openSecurity(item)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") openSecurity(item); }}><td className="rank-cell">{item.rank > 0 ? item.rank : "—"}</td><td><div className="table-security"><strong>{item.securityName}</strong><small>{item.securityCode}</small></div></td><td><span className={`priority-chip ${priorityClass(item.priority)}`}>{item.priority}</span></td><td><div className="score-cell"><strong>{(item.score * 100).toFixed(1)}</strong><span><i style={{ width: `${item.score * 100}%` }} /></span></div></td><td>{item.peTtm === null ? "—" : item.peTtm.toFixed(2)}</td><td>{formatMarketCap(item.marketCap)}</td><td>{formatPercent(item.netProfitMargin)}</td><td>{formatPercent(item.operatingCashFlowMargin)}</td><td><span className={`eligibility ${item.eligible ? "passed" : "failed"}`}><i />{item.eligible ? "通过" : "未通过"}</span></td></tr>)}</tbody></table></div><div className="pagination"><span>第 {page} / {totalPages} 页</span><div><button disabled={page === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>上一页</button><button disabled={page === totalPages} onClick={() => setPage((current) => Math.min(totalPages, current + 1))}>下一页</button></div></div></div>
        </section>}

        {view === "portfolio" && <section className="dashboard-view">
          <div className="section-title-row"><div><p className="section-eyebrow">PORTFOLIO · {portfolio.asOfDate}</p><h1>我的持仓</h1><p>持仓直接读取 GitHub main；行情缺失时保持空值，不做推算。</p></div><span className={`source-badge ${portfolio.liveData?.status === "fallback" ? "warning" : ""}`}><i />{portfolio.liveData?.status === "fallback" ? portfolio.liveData.message : "GitHub main 已同步"}</span></div>
        ...
        {view === "content" && <section className="dashboard-view">
          <div className="section-title-row"><div><p className="section-eyebrow">RESEARCH LIBRARY · {research.liveData?.latestDate ?? research.asOfDate}</p><h1>日报、新闻、公告与研究报告</h1><p>在一个内容库里搜索、筛选和阅读已归档材料，并随时回到原始来源。</p></div><div className="content-counts"><span>新闻 {research.coverage.newsCount}</span><span>公告 {research.coverage.announcementCount}</span><span>日报 {research.coverage.dailyCount}</span><span>报告 {research.coverage.reportCount}</span></div></div>
