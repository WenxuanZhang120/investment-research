#!/usr/bin/env python3
"""Search iWencai announcements and archive the unchanged response payload."""
...
def _api_key() -> str:
    ...
    if not value:
        raise AnnouncementCollectionError("IWENCAI_API_KEY is not set")
    return value
...
def _request(*, query: str, size: int, timeout: int, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise AnnouncementCollectionError(
            f"iWencai HTTP {error.code}: {body_text or error.reason}"
        ) from error
    except urllib.error.URLError as error:
        raise AnnouncementCollectionError(
            f"iWencai network error: {error.reason}"
        ) from error
    try:
        payload = json.loads(raw_body.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise AnnouncementCollectionError("iWencai returned non-JSON data") from error
    if not isinstance(payload, dict):
        raise AnnouncementCollectionError("iWencai response root must be an object")
    return payload
...
def collect_announcements(
    ...
    *,
    ...
    if not isinstance(query, str) or not query.strip():
        raise AnnouncementCollectionError("query must be non-empty")
    if size < 1 or timeout < 1:
        raise AnnouncementCollectionError("size and timeout must be positive")
    payload = request(
        query=query,
        size=size,
        timeout=timeout,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Archive an iWencai announcement search.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        destination = collect_announcements(
            args.query,
            size=args.size,
            timeout=args.timeout,
            raw_root=args.raw_root,
        )
    except (AnnouncementCollectionError, OSError, TypeError, ValueError) as error:
